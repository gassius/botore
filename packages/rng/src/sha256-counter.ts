...
const DRAW_BYTES = 8;
/** Draws per hashed block: 32 bytes / 8 bytes per draw. */

interface Sha256RngState {
  readonly algorithm: typeof SHA256_ALGORITHM;
  readonly key: Uint8Array; // 32 bytes
  counter: number; // logical block index
  buffer: bigint[]; // pre-drawn values, FIFO
}

function drawBlock(state: Sha256RngState): void {
...
export function createSha256Rng(key: Uint8Array): Rng & { snapshotKeyHex(): string } {
  if (key.length === 0 || key.length > 32) {
    throw new Error(`sha256-counter-v1 key must be 1..32 bytes, got ${key.length}`);
  }
  const padded = new Uint8Array(32);
  padded.set(key, 0);
  const state: Sha256RngState = {
  ...
  const nextRaw = (): bigint => {
    ...
    return state.buffer.shift() as bigint;
  };
